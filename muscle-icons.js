 ...
 * violet → orange de l'app (#muscleIconGrad). Les zones surlignées sont
 * détaillées (plusieurs faisceaux/lobes, lignes de fibres, séparations)
 * plutôt qu'un simple blob uni, pour un rendu réaliste — et portent un
 * léger glow (#muscleGlow) qui ne s'applique qu'à la partie colorée
 * (jamais à la silhouette au repos) pour un rendu premium. Le tout reste
 * strictement contenu dans le contour du corps grâce aux clip-paths
 * partagés (#torsoClip / #armClip / #hipClip / #calfClip, définis une
 * seule fois dans app.html, aux côtés du filtre #muscleGlow).
 *
 * Usage :
...
const MUSCLE_ICONS = {
  ...
  'Pectoraux': `
    ${TORSO}
    <g clip-path="url(#torsoClip)" filter="url(#muscleGlow)">
  ...
  'Quadriceps': `
    ${HIP}
    <g clip-path="url(#hipClip)" filter="url(#muscleGlow)">
  ...
  'Mollets': `
    ${CALF}
    <g clip-path="url(#calfClip)" filter="url(#muscleGlow)">
